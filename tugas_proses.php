...
$tujuan = htmlentities($_POST['tujuan']);
 if(!empty($nama_pemohon) && !empty($nama_perusahaan) && !empty($tujuan) ) {
		include "dbConnect.php";
			$tanggal = date("d-M-Y");
			$hasil = mysql_query("INSERT INTO surat_tugas(nama_pemohon,nama_perusahaan,tujuan,tanggal) 
				VALUES('$nama_pemohon','$nama_perusahaan','$tujuan','$tanggal')");
			if ($hasil){
				
				?>

				<script language="JavaScript">alert('sukses');
				document.location='formsurattugas.html'</script>
				<?php
			}
			else{ 
				...
				document.location='formsurattugas.html?page=login'</script>
				<?php
			}
		}
...
class pdf extends FPDF{
	...
	function idMahasiswa($nama_pemohon, $nama_perusahaan, $tujuan){
	        $this->Ln(2);
	        $this->SetFont('Times','',12);
	        $this->Cell(15);
...
$pdf->body1($teks);
$pdf->idMahasiswa($nama_pemohon, $nama_perusahaan, $tujuan);
//bodi surat 2
